...
- **Implementation approach:** one `useEffect` calling `track.animate([...],{duration,easing:'linear',iterations:Infinity})` then `anim.playbackRate = 80`, after measuring the authored `ul.list-2`'s own width (corrected after review: `.loop-sizer`'s box is only the viewport width, 1440/768/390, because of its inline `min-width:100%`). Re-measure on resize.
...
- The origin ships these as `src="data:image/svg+xml,…"` placeholders + `data-sd-img-src="…png"` (hidden by `img[data-sd-img-src]{display:none}`) and the Studio runtime swaps in the `_small.webp` at DPR 1; the clone should point `src` at the local FULL-WIDTH `s-1440x790_v-fms_webp_<uuid>.webp` (corrected after review: the ≤768 rule `width: calc(32% - var(--gap-h)*0.68)` is cyclic inside the intrinsically-sized sizer, so the card falls back to the resource's intrinsic width — the 600px `_small` files would give a 7552px track instead of the origin's 17632) and drop the `data-sd-img-src` / `<noscript>` pair.
